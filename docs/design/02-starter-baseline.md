...
1. Provisioned a scoped IAM user (`huntbot-cgep-capstone`) in the HPS AWS account (`837009194688`) with policies attached for API Gateway, Lambda, S3, DynamoDB, VPC, KMS, CloudTrail, IAM, and CloudWatch — no `AdministratorAccess`. Access key stored in 1P + on-disk profile.
